...
export default function Badge() {
  ...
  useEffect(() => {
    ...
    //console.log(process.env.REACT_APP_SERVER_BASE_URL, process.env.REACT_SERVER_BASE_URL)
        ...
        currentBadges.map((badge, index) => (
          <tr key={index}>
            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
              <img src={`http://localhost:3000${badge.imageUrl}`} alt={`Badge ${index}`} className="h-10 w-10 rounded-full" />
            </td>
          </tr>
        ))
      ) : (
