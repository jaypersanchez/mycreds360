...
const contractAddress = '0x3cB0c5Ec0A4ffd9993A9F0A0914FAA6206a7b10d'
const wallet_address = '0x7e6C8567198ab3580bb8A655c089eFb2BE2d6CCc'
const privateKey = '0x102805fbc4ea95f77c0173da2af4e70966c79c4dcd24b5e4f6335a4323b91b1b' 
...
/*
* This endpoint will return all courses that are available and have been assigned a badge
* image
*/
app.get('/badge-courses', async (req, res) => {
    const query = 'SELECT id, course_name, description, badge FROM courses';
...
app.post('/create-student-badge', async (req, res) => {
    const { student_id, course_id, course_name, date_completion, badgeImageUrl } = req.body;
    console.log(student_id, course_id, course_name, date_completion, badgeImageUrl)
    const query = `
        INSERT INTO mycreds360.badges 
        (user_id, course_id, course_name, date_completion, created_at, updated_at, badgeimageurl) 
        VALUES (?, ?, ?, ?, NOW(), NOW(),?)
    `;
    ...
    db.pool.getConnection((err, connection) => {
        ...
        connection.query(query, [student_id,course_id, course_name, date_completion,badgeImageUrl], (err, results) => {
            // Release the connection back to the pool
            connection.release();
            if (err) {
                console.error('Error executing query:', err);
...
// This route will generate the badge for a specific course that is to be issued to a student
...
app.get('/studentbadges', async (req, res) => {
    const userId = req.query.userId;
    console.log(userId)
    const query = 'SELECT user_id,course_name, date_completion, badgeimageurl FROM badges WHERE user_id = ?';
